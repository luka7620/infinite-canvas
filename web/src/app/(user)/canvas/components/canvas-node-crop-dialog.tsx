            ...
            <div className="space-y-4">
                <div className="flex justify-center">
                    <div ref={boxRef} className="relative inline-block max-w-full overflow-hidden rounded-lg bg-black select-none">
                        ...
                        <CropMask crop={crop} />
                        <div className="absolute cursor-move border-2 border-primary shadow-sm" style={cropStyle(crop)} onPointerDown={(event) => startDrag("move", event)}>
                            ...
                            {handles.map((handle) => (
                                <button key={handle} type="button" className="absolute size-3 rounded-full border border-background bg-primary" style={handleStyle(handle)} onPointerDown={(event) => startDrag("resize", event, handle)} aria-label="调整裁剪框" />
                            ))}
                        </div>
                    </div>
                </div>
