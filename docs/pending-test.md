...
- 新增用户公开作品画廊：后端模型通道生成图片会写入生成图片记录，覆盖生图工作台、画布节点生成和图片编辑场景；生图工作台可将有生成记录的结果发布到画廊，发布时可选择是否公开提示词。
- 新增 `/gallery` 公开画廊页面，支持按关键词和标签查看已公开作品；未选择分享提示词的作品前台不展示提示词，较长提示词默认折叠展示。
